...
export default function DataDescriptor({ form, onChange, data }) {
  ...
  useEffect(() => {
    ...
    // 2 & 3. author + affiliation rows — stay in sync with Contributors/
    // Custodian automatically until the user manually edits them.
    // `authorsAutoGenerated`/`affiliationsAutoGenerated` default to true
    // (unset) so existing saved data keeps auto-syncing too.
    // Computed together and emitted in a single onChange call — two separate
    // calls in the same effect tick would race and the second would clobber
    // the first's update, since each call replaces the whole dataDescriptor
    // object rather than deep-merging it.
    const authorsAuto      = dd.authorsAutoGenerated      !== false
    const affiliationsAuto = dd.affiliationsAutoGenerated !== false

    const nextAuthors = authorsAuto
      ? buildAutoAuthors(data, personMap)
      : (dd.authors || []).map(a => ({
          ...a,
    ...
    const nextAffiliations = affiliationsAuto
      ? buildAutoAffiliations(data)
      : (dd.affiliations_list || [])

    setAuthors(nextAuthors)
    setAffiliations(nextAffiliations)

    const authorsChanged      = authorsAuto &&
      JSON.stringify(nextAuthors) !== JSON.stringify(dd.authors || [])
    const affiliationsChanged = affiliationsAuto &&
      JSON.stringify(nextAffiliations) !== JSON.stringify(dd.affiliations_list || [])

    if (authorsChanged || affiliationsChanged) {
      emitAll(nextAuthors, nextAffiliations, undefined, {
        authorsAutoGenerated:      authorsAuto,
        affiliationsAutoGenerated: affiliationsAuto,
      })
    }
  ...
  const emitAll = (newAuthors, newAffs, formVals, extra = {}) => {
    ...
    onChange({
      dataDescriptor: {
        ...vals,
        // carry the auto/customized flags forward by default — only an
        // explicit `extra` override (from a manual edit, or the sync
        // effect confirming an auto rebuild) should change them
  ...
  // Same rule as authors: a manual edit stops the auto-sync from Custodian.
  const addAffiliation = () => {
    ...
    setAffiliations(next); emitAll(authors, next, undefined, { affiliationsAutoGenerated: false })
  }
  ...
  // Calls the backend, which calls Mistral server-side (the API key never
  // reaches the browser). Sends the researcher's own answers so far — the
  // model is instructed not to invent facts, only combine what's given.
  const generateSummary = async () => {
    setSummaryError('')
    const vals = form.getFieldsValue().dataDescriptor || {}
    const payload = {
      title:             data.dataset1?.dataTitle || vals.title || '',
      briefSummary:      data.dataset1?.briefSummary || '',
      whatAreTheData:    vals.whatAreTheData || '',
      scientificContext: vals.scientificContext || '',
    ...
    setSummarizing(true)
    try {
      const res = await fetch('/api/mistral/generate-summary', {
        method: 'POST',
      ...
      if (!res.ok) {
        setSummaryError(result.error || 'Could not generate a summary — please try again.')
        return
      }
      form.setFieldValue(['dataDescriptor', 'summary'], result.summary)
      emitAll(authors, affiliations, { ...vals, summary: result.summary })
    } catch (err) {
      console.error('generateSummary failed:', err)
      setSummaryError('Could not reach the server — please try again.')
    } finally {
  ...
  return (
    <div>
      ...
      <AntForm
        form={form}
        layout="vertical"
        initialValues={{ dataDescriptor: computeFieldValues(data) }}
      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ marginBottom: 4 }}>
            <span style={{ fontWeight: 500, fontSize: 14 }}>
              Affiliations
              {hasAutoAffiliations && <PrefilledBadge />}
            </span>
          </div>
          {hasAutoAffiliations && <PrefilledHint />}
          <span style={EXTRA}>
            ...
            label={
              <div style={{
                ...
                <Button size="small" icon={<ThunderboltOutlined />}
                  loading={summarizing} onClick={generateSummary}>
                  Generate with AI
                </Button>
              </div>
            }
            name={['dataDescriptor', 'summary']}
            extra={<span style={EXTRA}>
              A short paragraph combining your answers above into one flowing summary. Optional —
              if left empty, the generated Data Descriptor will build one automatically from your answers.
            </span>}
          >
            <TextArea autoSize={{ minRows: 3, maxRows: 8 }}
              placeholder='Click "Generate with AI" to draft a summary from your answers above, or write your own.' />
          </AntForm.Item>
          {summaryError && (
            <div style={{ color: '#d32f2f', fontSize: 12, marginTop: -12 }}>{summaryError}</div>
          )}
        </div>
